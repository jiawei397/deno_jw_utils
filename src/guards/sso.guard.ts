    ...
    return class Guard implements CanActivate {
        ...
        async validateRequest(context: Context) {
            try {
                ...
                if (request.userInfo) {
                    logger.debug(`【sso guard】上一个guard中已经有用户信息：${stringify(this.getSimpleUserInfo(request.userInfo))}`);
                    return true;
                }
                const userInfo = await this.getSSO(request);
                const simpleInfo = this.getSimpleUserInfo(userInfo);
                if (!userInfo.internal && (ssoAllowAllUsers ?? Deno.env.get('ssoAllowAllUsers') !== 'true')) { // 外来用户
